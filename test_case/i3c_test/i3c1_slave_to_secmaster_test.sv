`ifndef I3C1_SLAVE_TO_SECMASTER_TEST__SV
`define I3C1_SLAVE_TO_SECMASTER_TEST__SV
class i3c1_slave_to_secmaster_test extends i3c_base_test;
   `uvm_component_utils(i3c1_slave_to_secmaster_test)
   i3c_vip_secmst_ibi_sequence secmst_seq;

   extern function new(string name = "i3c1_slave_to_secmaster_test", uvm_component parent = null);
   extern virtual function void build_phase(uvm_phase phase);
   extern virtual task main_phase(uvm_phase phase);

endclass: i3c1_slave_to_secmaster_test
//-------------------------------------------------------------------------
function i3c1_slave_to_secmaster_test::new(string name = "i3c1_slave_to_secmaster_test", uvm_component parent = null);
    super.new(name, parent);
       
endfunction: new
//-------------------------------------------------------------------------
function void i3c1_slave_to_secmaster_test::build_phase(uvm_phase phase);
...
//-------------------------------------------------------------------------
task i3c1_slave_to_secmaster_test::main_phase(uvm_phase phase);
    bit[31:0] wdata ;
    bit[31:0] rdata ;
    bit[7:0] tx_data_q[$];
...
//        wdata[31]='h1;
    ...
    fork
      begin       
        ...
        // MR        
        noc_reg_read(`I3C1_BASE+`SLV_EVENT_STATUS, rdata );
        `uvm_info(get_type_name(), $sformatf("[HURYDEBUG1105] mr_en data = 0x%h", rdata), UVM_LOW)

        if(rdata[1] == 1'b1) begin
          //set mr 
          noc_reg_read(`I3C1_BASE+`SLV_INTR_REQ, rdata );
          rdata[3]='h1;
          noc_reg_write(`I3C1_BASE+`SLV_INTR_REQ, rdata );
...
//      wdata[6:0]  = static_addr; //dev static addr 
        noc_reg_write(`I3C1_BASE+`DEV_ADDR_TABLE_LOC1, wdata);
...
endtask: main_phase
//-------------------------------------------------------------------------
`endif //I3C1_SLAVE_TO_SECMASTER_TEST__SV
