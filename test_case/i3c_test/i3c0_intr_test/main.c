...
void mcu_i3c0_intr_handler(void) {
    ...
    wait(100);
    intr_status = read32(MCU_SUB_I3C0_BASE_ADDR + INTR_STATUS);
    cpu_info("==i3c0 intr status before clear :0x%x==\n", intr_status);
   ...
   write32(MCU_SUB_I3C0_BASE_ADDR + INTR_STATUS, I3C_RX_INTR_MASK);
   intr_status = read32(MCU_SUB_I3C0_BASE_ADDR + INTR_STATUS);
   cpu_info("==i3c0 intr status after clear :0x%x==\n", intr_status);
   int_flg++;
}
