`ifndef I2C1_SLAVE_TRANSMIT_TRANS_TEST__SV
`define I2C1_SLAVE_TRANSMIT_TRANS_TEST__SV

class i2c1_slave_transmit_trans_test extends i3c_base_test;
   `uvm_component_utils(i2c1_slave_transmit_trans_test)

   i3c_vip_mst_read_sequence mst_seq;

   extern function new(string name = "i2c1_slave_transmit_trans_test", uvm_component parent = null);
   extern virtual function void build_phase(uvm_phase phase);
   extern virtual task main_phase(uvm_phase phase);

endclass: i2c1_slave_transmit_trans_test
//-------------------------------------------------------------------------
function i2c1_slave_transmit_trans_test::new(string name = "i2c1_slave_transmit_trans_test", uvm_component parent = null);
    super.new(name, parent);
       
endfunction: new
//-------------------------------------------------------------------------
function void i2c1_slave_transmit_trans_test::build_phase(uvm_phase phase);
...
//-------------------------------------------------------------------------
task i2c1_slave_transmit_trans_test::main_phase(uvm_phase phase);
    bit[31:0] wdata ;
    bit[31:0] rdata ;
    bit[7:0] tx_data_q[$];
    ...
    fork
        begin         
           ...
           //1.set slave
            ...
            `uvm_info(get_type_name(), $sformatf("[HURYDEBUG1014]444444intr_status data = 0x%h", rdata), UVM_LOW)
            while(rdata[3] != 1'b1) begin 
                noc_reg_read(`I3C1_BASE+`INTR_STATUS, rdata);
                `uvm_info(get_type_name(), $sformatf("[HURYDEBUG1014]555555 intr_status data = 0x%h", rdata), UVM_LOW)
            end 

            `uvm_info(get_type_name(), $sformatf("[HURYDEBUG1014]666666  ENTER  tx command 2222  "), UVM_LOW)
           ...
           //-----------------------------------------------------------
           i3c_slave_txdata_to_vip_master_pre_tr(1, tx_data_q);
...
endtask: main_phase
//-------------------------------------------------------------------------
`endif //I2C1_SLAVE_TRANSMIT_TRANS_TEST__SV
