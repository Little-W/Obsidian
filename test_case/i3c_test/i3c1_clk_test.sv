`ifndef I3C1_CLK_TEST__SV
`define I3C1_CLK_TEST__SV


class i3c1_clk_test extends i3c_base_test;
   `uvm_component_utils(i3c1_clk_test)

   virtual crg_user_intf    crg_vif;
    
   extern function new(string name = "i3c1_clk_test", uvm_component parent = null);
...
endclass: i3c1_clk_test
//-------------------------------------------------------------------------
function i3c1_clk_test::new(string name = "i3c1_clk_test", uvm_component parent = null);
    super.new(name, parent);
       
endfunction: new
//-------------------------------------------------------------------------
function void i3c1_clk_test::build_phase(uvm_phase phase);
...
//-------------------------------------------------------------------------
task i3c1_clk_test::default_val_chk(RegTypeDef_t reg_struct[], bit[31:0] base_addr);
  logic [31:0] rdata;
...
endtask
//-------------------------------------------------------------------------
task i3c1_clk_test::reset_dev_regs(RegTypeDef_t reg_struct[], bit[31:0] base_addr);
  logic [31:0] rdata;
  for(int i = 0; i < reg_struct.size; i++) begin
...
endtask
//-------------------------------------------------------------------------
task i3c1_clk_test::write_reg_chk(RegTypeDef_t reg_struct[], bit[31:0] base_addr, bit[31:0] val);
  ...
  for(int i = 0; i < reg_struct.size; i++) begin
      if(reg_struct[i].OFFSET != 32'h3c && reg_struct[i].OFFSET != 32'h0 && reg_struct[i].OFFSET != 32'h4 ) begin
        ...
        if(rdata !== rdata_exp) begin
          `uvm_error(get_name(), $sformatf("[i3c1_wr_reg_test]: reg_name:%s, addr=0x%0h, rdata_old=0x%0h, wdata=0x%0h, exp=0x%0h, act=0x%0h", reg_struct[i].reg_name, base_addr + reg_struct[i].OFFSET, rdata_old, val, rdata_exp, rdata))
        end  
      end
...
endtask
 ...
 //---------------------------------------------------------------------------------------------     
task i3c1_clk_test::main_phase(uvm_phase phase);
    bit[31:0] wdata ;
    bit[31:0] rdata ;
    phase.raise_objection(this);
    super.main_phase(phase);

	  crg_vif.mcu_i3c1_pclk_assert_vif.rst = 1'b1;
