...
//   test_intr_conn_per_core(`MCU_NPU_MAILBOX_INTR47_PATH, `MCU_NPU_MAILBOX_INTR47_IRQ_ID, "MCU_NPU_MAILBOX_INTR47", 3);
  ...
  while(!(die_id == 3 && chip_id == 3)) begin

    uvm_hdl_force("hdl_top.T100_TOP_u.DIE_ID0",i[0]);
    uvm_hdl_force("hdl_top.T100_TOP_u.DIE_ID1",i[1]);

    uvm_hdl_force("hdl_top.T100_TOP_u.CHIP_ID0",i[2]);
    uvm_hdl_force("hdl_top.T100_TOP_u.CHIP_ID1",i[3]);

    `uvm_info(get_type_name(),
                $psprintf("wait ini_done_flag"),
                UVM_LOW);

    while(init_flag == 0) begin
        uvm_hdl_read("hdl_top.T100_TOP_u.PD_AON_SUB_0.AON_EFUSE_WRAPPER_0.ini_done_flag", init_flag);
        #10ns;
    end
    ...
    uvm_hdl_read("hdl_top.T100_TOP_u.PD_AON_SUB_0.AON_EFUSE_WRAPPER_0.die_id[1:0]", die_id);
    uvm_hdl_read("hdl_top.T100_TOP_u.PD_AON_SUB_0.AON_EFUSE_WRAPPER_0.chip_id[1:0]", chip_id);
    

    `uvm_info(get_type_name(),
                  $psprintf("die_id = %d, chip_id=%d",
                            die_id, chip_id),
                  UVM_LOW);

    `uvm_info(get_type_name(),
                $psprintf("wait ini_done_flag done"),
                UVM_LOW);


    uvm_hdl_release("hdl_top.T100_TOP_u.DIE_ID0");
    uvm_hdl_release("hdl_top.T100_TOP_u.DIE_ID1");

    uvm_hdl_release("hdl_top.T100_TOP_u.CHIP_ID0");
    uvm_hdl_release("hdl_top.T100_TOP_u.CHIP_ID1");
    uvm_hdl_force("hdl_top.T100_TOP_u.POR_RST_N",0);
    #5us;
    uvm_hdl_release("hdl_top.T100_TOP_u.POR_RST_N");
    uvm_hdl_force("hdl_top.T100_TOP_u.POR_RST_N",1);
