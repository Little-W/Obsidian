...
`ifndef CPU_CORE1_WFI_DEEP_TEST_SV
`define CPU_CORE1_WFI_DEEP_TEST_SV

class cpu_core1_wfi_deep_test extends chip_noc_base_test;
    `uvm_component_utils(cpu_core1_wfi_deep_test)
    extern function new(string name = "cpu_core1_wfi_deep_test", uvm_component parent = null);
    extern virtual task main_phase(uvm_phase phase);
endclass

function cpu_core1_wfi_deep_test::new(string name = "cpu_core1_wfi_deep_test", uvm_component parent = null);
    super.new(name,parent);

endfunction:new

task cpu_core1_wfi_deep_test::main_phase(uvm_phase phase);
    bit[7:0] clk_flg = 0;
    int start_time;
    int end_time;
    int freq = 0;
