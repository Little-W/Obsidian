...
`ifndef CPU_CORE1_WFE_TEST_SV
`define CPU_CORE1_WFE_TEST_SV

class cpu_core1_wfe_test extends chip_noc_base_test;
    `uvm_component_utils(cpu_core1_wfe_test)
    extern function new(string name = "cpu_core1_wfe_test", uvm_component parent = null);
    extern virtual task main_phase(uvm_phase phase);
endclass

function cpu_core1_wfe_test::new(string name = "cpu_core1_wfe_test", uvm_component parent = null);
    super.new(name,parent);

endfunction:new

task cpu_core1_wfe_test::main_phase(uvm_phase phase);
    bit[7:0] clk_flg = 0;
    int start_time;
    int end_time;
    int freq = 0;
    ...
    repeat(1000)begin
        ...
        if(core_clk != 0)begin
            `uvm_error(get_type_name(), "core1_core_clk bust be low after cpu exe WFE");
        end
        #1ns;
    end
    send_label(55);
