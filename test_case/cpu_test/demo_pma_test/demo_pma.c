...
int main(void)
{
    ...
    if (!check_aligned(pma_cfg.region_base, pma_cfg.region_size)) {
        cpu_info("Error: Region base address 0x%x must be aligned to region granule 0x%x and region size 0x%x!\n", pma_cfg.region_base, REGION_GRANULE, pma_cfg.region_size);
        return -1;
    }

    GetDCacheInfo(&cacheinfo_type);
    if (COL_SIZE != cacheinfo_type.linesize) {
        cpu_info("\nWarning: change COL_SIZE to %0d to make them same!\n", cacheinfo_type.linesize);
    }

    cpu_info("DCache Linesize is %0d bytes, ways is %0d, setperway is %0d, total size is %0d bytes\n", cacheinfo_type.linesize, \
            cacheinfo_type.ways, cacheinfo_type.setperway,cacheinfo_type.size);

    array_init();

    cpu_info("array_test size: %0d * %0d bytes, addr: 0x%x\n", ROW_SIZE, COL_SIZE, (unsigned long)&array_test);
    EnableICache();
    ...
    //cpu_info("Region type: 0x%x,region base addr: 0x%lx, region size: 0x%lx, region status: %0d\n",\
