...
{
    cpu_info("Initialize timer and start timer interrupt periodically\n\r");
...
void eclic_mtip_handler(void)
{
    ...
    cpu_info("-------------------\r\n");
    cpu_info("[IN TIMER INTERRUPT]timer interrupt hit %d times\r\n", int_t_cnt++);

    cpu_info("[IN TIMER INTERRUPT]trigger software interrupt\r\n");
#if SWIRQ_INTLEVEL_HIGHER == 1
    cpu_info("[IN TIMER INTERRUPT]software interrupt will run during timer interrupt\r\n");
#else
    cpu_info("[IN TIMER INTERRUPT]software interrupt will run when timer interrupt finished\r\n");
#endif
...
__INTERRUPT void eclic_msip_handler(void)
{
    ...
    cpu_info("[IN SOFTWARE INTERRUPT]software interrupt hit %d times\r\n", int_sw_cnt++);
    cpu_info("[IN SOFTWARE INTERRUPT]software interrupt end\r\n");
...
int main(int argc, char** argv)
{
    ...
    __disable_irq();
    cpu_info("ECLIC Demo finished sucessfully in %d loops\n", RUN_LOOPS);
