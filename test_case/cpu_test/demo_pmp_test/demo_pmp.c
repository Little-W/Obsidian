...
{
    cpu_info("----protected_execute succeed!----\r\n");

    cpu_info("Attempting to read protected_data[0]\r\n");
    cpu_info("protected_data[0]: 0x%0X succeed\r\n", protected_data[0]);

    cpu_info("Attempting to write protected_data[0]\r\n");
    protected_data[0] = 0xFF;

    cpu_info("Won't run here if violates L R\\W\\X permission check!\r\n");
}
...
static void pmp_violation_fault_handler(unsigned long mcause, unsigned long sp)
{
    ...
    switch (mcause & MCAUSE_CAUSE) {
        case InsAccFault_EXCn:
            cpu_info("Instruction access fault occurs, cause: 0x%lx, epc: 0x%lx\r\n", exc_frame->cause, exc_frame->epc);
            break;
        case LdFault_EXCn:
            cpu_info("Load access fault occurs, cause: 0x%lx, epc: 0x%lx\r\n", exc_frame->cause, exc_frame->epc);
            break;
        case StAccessFault_EXCn:
            cpu_info("Store/AMO access fault occurs, cause: 0x%lx, epc: 0x%lx\r\n", exc_frame->cause, exc_frame->epc);
            break;
        default: break;
    }
...
int main(void)
{
    ...
    cpu_info("------PMP demo with trigger condition %d------\r\n", TRIGGER_PMP_VIOLATION_MODE);
    ...
    cpu_info("Get pmp entry: index %d, prot_out: 0x%x, addr_out: 0x%lx, order_out: %lu\r\n", \
    ...
    cpu_info("Attempting to fetch instruction from protected address\n");
    fncptr();
#else
    cpu_info("[ERROR]__PMP_PRESENT must be defined as 1 in <Device>.h!\r\n");
#endif
    simend();
    while(1);
    //return 0;
