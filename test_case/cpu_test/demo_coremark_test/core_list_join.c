...
list_head* core_list_init(ee_u32 blksize, list_head* memblock, ee_s16 seed)
{
    ...
    list = core_list_mergesort(list, cmp_idx, NULL);
#if CORE_DEBUG
    cpu_info("Initialized list:\n");
    finder = list;
    while (finder) {
        cpu_info("[%04x,%04x]", finder->info->idx, (ee_u16)finder->info->data16);
        finder = finder->next;
    }
    cpu_info("\n");
#endif
