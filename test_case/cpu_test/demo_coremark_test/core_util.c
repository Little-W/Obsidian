...
ee_u8 check_data_types()
{
    ee_u8 retval = 0;
    if (sizeof(ee_u8) != 1) {
        cpu_info("ERROR: ee_u8 is not an 8b datatype!\n");
        retval++;
    }
    if (sizeof(ee_u16) != 2) {
        cpu_info("ERROR: ee_u16 is not a 16b datatype!\n");
        retval++;
    }
    if (sizeof(ee_s16) != 2) {
        cpu_info("ERROR: ee_s16 is not a 16b datatype!\n");
        retval++;
    }
    if (sizeof(ee_s32) != 4) {
        cpu_info("ERROR: ee_s32 is not a 32b datatype!\n");
        retval++;
    }
    if (sizeof(ee_u32) != 4) {
        cpu_info("ERROR: ee_u32 is not a 32b datatype!\n");
        retval++;
    }
    if (sizeof(ee_ptr_int) != sizeof(int*)) {
        cpu_info("ERROR: ee_ptr_int is not a datatype that holds an int pointer!\n");
        retval++;
    }
    if (retval > 0) {
        cpu_info("ERROR: Please modify the datatypes in core_portme.h!\n");
    }
    return retval;
}
