...
struct multi_table_t *kernel_mmu_table ;
static uint64_t mmu_alloc_index = 1;

void page_mem_int (void)
{
    kernel_mmu_table = (struct multi_table_t*)aligned_alloc(sizeof(struct multi_table_t),4096);
}
    ...
    while (1)
    {
        if (sz_temp > 0)
        {
            ...
            else
            {
                ...
                cpu_error("pte size or address is overlap");
            }
