...
双hart一致性错误通常不是单个功能完全缺失，而是两个本来合法的操作在同一周期或不同AXI通道上交错。以下内容记录已经确认的并发问题、形成原因、处理方法和验证重点，供调整Cache容量、未完成事务数或寄存级时逐项检查。

#### 5.6.1 相同字的加载后存储

- 问题表现：同一hart已经发出加载，但R响应尚未返回时，较晚的存储可能先完成AW和W。另一个hart可能先看到新写值，而本hart的较早加载仍使用旧值；在共享指针、自旋状态或链表操作中，这种次序会破坏程序依赖关系。
- 形成原因：AXI读写通道彼此独立，轻度乱序单发射、有限乱序完成允许存储写地址和写数据先于较早加载的读响应完成。仅依靠指令发射次序不能保证共享侧观察次序。
- 处理方法：LSU的两项读记录保存每笔未完成加载的自然对齐字地址。新普通存储比较当前有效读记录，只等待地址相同的较早加载；地址不同的读写仍可并行。队首R在当前周期完成时，该项立即不再阻止同周期存储，避免额外空等一拍。
- 验证重点：连续执行相同字和不同字的加载后存储；让R响应与存储解除暂停发生在同一周期；检查双hart链表、自旋同步及共享指针程序不会形成错误循环。

#### 5.6.2 跨hart写与LSU L0 Cache未完成加载

- 问题表现：另一个hart完成写地址握手并触发监听后，本地已经发出的旧读响应仍可能稍后返回。如果该响应再次填入LSU L0 Cache，下一次加载会重新取得已经失效的数据。
- 形成原因：清除当前有效位只能处理监听到达时已经存在的副本，无法取消此前已进入DCache或共享存储的读请求。
- 处理方法：LSU的每项读记录同时保存组号、标签和填充资格。监听地址与记录完全匹配时，记录被标记为不得填充；R响应仍用于完成原加载，但不会设置LSU L0 Cache有效位，也不会更新标签和数据阵列。
- 验证重点：分别覆盖监听早于R、监听与R同周期、同一地址连续监听，以及相同组不同标签的未完成读。

#### 5.6.3 监听与LSU L0 Cache流水命中

- 问题表现：命中已经进入流水寄存级后，监听在下一周期到达；如果只依靠时钟沿清除有效位，流水中的命中仍可能使用一次旧数据。
- 形成原因：标签查询结果和有效位清除不在同一组合阶段，已经寄存的命中不会自动感知当前周期的新监听。
- 处理方法：发射模块保留流水中加载的组号和标签，并在监听当拍直接比较。完全匹配时立即取消该命中；相同组不同标签不取消，以免无故降低64项、1路结构的命中率。
- 验证重点：监听与查询同周期、监听晚于查询一周期、相同组不同标签，以及流水暂停期间收到监听。

#### 5.6.4 DCache写探测与监听清除

- 问题表现：部分字节写可能采用前一次标签探测结果，更新错误路；严重时同一组两路出现相同标签，未写字节与新写字节来自不同数据。
- 形成原因：标签读取和监听清除共用存储端口。若监听清除优先，而控制逻辑仍把同周期写探测视为已经完成，本次写会错误使用前一次探测留下的结果。
- 处理方法：任意有效监听周期都不启动新的普通写探测。已经完成的写探测仅在监听使用相同组时取消；无关组的待提交更新继续保留。部分字节写只有在真实命中时才更新，未命中时只写共享存储，不在DCache中分配新项。
- 验证重点：无关组监听与部分字节写重叠、相同组监听与写探测重叠、重复标签检查，以及冲突后第一次普通读取必须从正确位置取得数据。

#### 5.6.5 DCache填充和本地更新与监听

- 问题表现：监听已经清除目标组后，较早的未命中R响应可能再次写入DCache；另一种错误是连续无关监听占用端口，使已经收到成功B响应的本地写更新被丢弃。
- 形成原因：读填充、本地写提交和监听清除都需要写标签、有效位或数据，三个操作可能在相邻周期争用相同存储端口。
- 处理方法：监听到达后，所有已发出但尚未返回的未命中读都禁止填充，R响应只完成原读。待提交本地写更新一直保存到端口可用；只有使用相同组的监听才取消与旧探测结果相关的更新。监听周期不启动新的普通标签操作。
- 验证重点：监听早于R、监听与B同周期、相同组和不同组的连续监听，以及写更新在多周期等待后仍能正确提交。

#### 5.6.6 来源CPU的旁路写

- 问题表现：原子、排他或突发写使另一个CPU清除了旧副本，但来源CPU自己的DCache仍可能保留写前数据；随后普通加载在来源CPU命中旧值。
- 形成原因：跨hart监听只发送给写请求来源之外的CPU，而旁路写不执行普通DCache命中更新，所以来源CPU既收不到监听，也没有新的Cache数据可提交。
- 处理方法：DCache接受旁路AW时登记本地目标组清除，待标签端口可用后执行。失败SC也允许保守清除，因为清除只会增加下一次读取，不会把失败写入的数据作为有效数据。
- 验证重点：先预热DCache，再执行AMO、成功SC、失败SC、排他写或突发写，最后以普通加载读取相同地址。

#### 5.6.7 紧密跳转循环与IPI

- 问题表现：从hart在无条件跳转循环中等待IPI（Inter-Processor Interrupt，处理器间中断）时，软件中断可能长期无法进入，从hart不能退出等待循环。
- 形成原因：如果CLINT在每一个BJP请求周期都推迟中断，无条件跳转会连续产生BJP请求，使中断一直没有进入机会。
- 处理方法：BJP尚未给出确定的下一PC时才推迟中断；已经得到有效跳转目标时，CLINT保存该PC并允许进入中断。原子操作和存储操作繁忙时继续推迟异常，避免把异常插入读改写过程。
- 验证重点：从hart紧密等待循环、hart0发送IPI、从hart进入中断、清除`MSIP`和执行`mret`返回。

#### 5.6.8 M1响应寄存级

- 问题表现：把AW、W、B、AR和R统一增加一级，会让普通存储在共享从端已经给出B响应后仍多等待一拍；CoreMark中大量存储使该固定开销累计成明显周期损失。
- 形成原因：请求方向需要寄存级切分较长路径，但B响应已经有Crossbar从端选择、数据一致性控制写事务记录和DCache写提交状态保证对应关系，再增加一级不会扩大未完成事务容量。
- 处理方法：M1主参数仍为1级且fall-through关闭；AW、W、AR和R保留1级，B单独设为0级。BID和BRESP保持完整，B通道暂停规则不变，响应仍由写事务记录送回正确hart和本地ID。
- 验证重点：M1访问各共享从端、B通道连续暂停、多个写请求使用不同ID、错误响应保持，以及运行时配置打印中的主参数与实际响应级数说明。
