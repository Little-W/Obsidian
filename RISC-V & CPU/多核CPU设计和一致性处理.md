...
`fence pred, succ`分别给出前驱集合与后续集合。集合中的`R`表示主存储读取，`W`表示主存储写入，`I`表示设备输入，`O`表示设备输出。其他hart或外部设备不能先观察到后续集合中的操作，再观察到前驱集合中的操作。

| 指令 | 建立的主要次序 | 常见用途 |
| --- | --- | --- |
| `fence rw, rw` | 之前的主存储读写先于之后的主存储读写 | 通用全栅栏，控制范围较大 |
| `fence rw, w` | 之前的主存储读写先于之后的主存储写 | 在写入状态标志前发布数据 |
| `fence r, rw` | 之前的主存储读先于之后的主存储读写 | 读取状态标志后获取数据 |
| `fence w, o` | 之前的主存储写先于之后的设备输出 | 写好DMA（Direct Memory Access，直接存储器访问）描述符后写MMIO启动寄存器 |
| `fence iorw, iorw` | 设备输入、设备输出和主存储读写均建立前后次序 | 需要同时控制主存储与MMIO访问时使用 |

原子指令的`aq`位使该原子操作先于同一访问域中的后续操作，`rl`位使同一访问域中的先前操作先于该原子操作，`aqrl`同时提供两种作用。主存储与MMIO属于不同访问域时，不能仅依赖`aq`或`rl`跨域排序，应使用包含`I`或`O`的`fence`。
