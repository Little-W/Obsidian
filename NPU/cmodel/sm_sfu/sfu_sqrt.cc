...
uint32_t sfu_sqrt(uint32_t x) {
  uint32_t rlt_f32;         // poly result
  uint32_t result;          // final result
  uint32_t exp_nobias = 0;  // real exp
  bool exp_op_sqrt = false; // add/sub exp_new
  uint32_t exp_new_sqrt = 0;
  ...
  if (sign) {
    if ((!exp_org) && (!frac)) {
      ...
      return packToF32UI(sign, 0, 0); //-0
    }
    cerr << "invalid operation for negative in sqrt()" << endl;
  ...
  if ((!exp_org) && frac) {
    ...
    if (exp_nobias % 2) {
      exp_new_sqrt = (exp_nobias >> 1) + 1;
    } else {
      exp_new_sqrt = (exp_nobias >> 1);
    }
    exp_op_sqrt = 1;
  } else {
    // normalized
  ...
  if (((man_ext >> 24) & 0x1) == 0x1 || ((man_ext >> 22) & 0x3) == 0x3) {
    seg_addr = 1;
  ...
  // get coef, poly
  info_coef_fp32 info = get_info_coef_fp32(sqrt_coef_info_f32, 56, coef_addr);
  ...
  uint32_t exp_rlt;
  if (exp_op_sqrt) {
    if (exp_poly > exp_new_sqrt) {
      exp_rlt = -exp_new_sqrt + exp_poly;
    } else if (exp_poly == exp_new_sqrt) {
      uint32_t new_frac = (1 << 23) | frac_rlt;
      exp_rlt = 0;
      frac_rlt = get_bits(new_frac, 1, 23);
    } else {
      exp_rlt = 0;
      frac_rlt = get_frac_shift(sign_rlt, frac_rlt, exp_new_sqrt, exp_poly);
    }
  } else {
    exp_rlt = exp_new_sqrt + exp_poly;
  }
  if (exp_rlt >= 255) {
    // cmodel_exceptionFlags |=  softfloat_flag_overflow; //FIXME:rtl not set
...
#if 0
  // use std float
  float x_in = ui32_to_float(x_flit); // x-left
  float ploy_result = poly_out_double(x_in, get_sqrt_coef_double(coef_addr));
  float rlt_f;
  if (exp_op_sqrt) {
    rlt_f = pow(2, -exp_new_sqrt) * ploy_result;
  } else {
    rlt_f = pow(2, exp_new_sqrt) * ploy_result;
  }
  cout << x_in << ", " << rlt_f << endl;
#endif
