...
static int mif_test_normal_read(void)
{
    ...
    TEST_CHECK(outputs.axi.araddr ==
               UINT64_C(0x100));
    TEST_CHECK(outputs.axi.arlen == 1u);
    TEST_CHECK(outputs.axi.arsize == 3u);
    TEST_CHECK(outputs.axi.arburst == 1u);
    TEST_CHECK(outputs.axi.arlock == 0u);
    TEST_CHECK(outputs.axi.arcache == 15u);
    TEST_CHECK(outputs.axi.arprot == 3u);
    TEST_CHECK(outputs.axi.arqos == 2u);
...
static int mif_test_normal_write(void)
{
    ...
    TEST_CHECK(outputs.axi.awaddr ==
               UINT64_C(0x200));
    TEST_CHECK(outputs.axi.awlen == 1u);
    ...
    TEST_CHECK(outputs.axi.wstrb == 0x0fu);
    TEST_CHECK(outputs.axi.wlast == 0u);
    held_data = outputs.axi.wdata;

    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.axi.wvalid != 0u);
    TEST_CHECK(outputs.axi.wdata == held_data);
    TEST_CHECK(outputs.axi.wstrb == 0x0fu);
...
static int mif_test_dfu_high_system_address_read(void)
{
    npu_mif_cycle_t model;
    npu_mif_cycle_inputs_t inputs;
    npu_mif_cycle_outputs_t outputs;
    ...
    TEST_CHECK(outputs.axi.araddr ==
               UINT64_C(0x80000));
    TEST_CHECK(outputs.axi.arlen == 0u);
    read_id = outputs.axi.arid;
    inputs.axi.arready = 0u;
...
static int mif_test_cross_4k(void)
{
    ...
    inputs.axi.rdata = UINT64_C(0xa0);
    inputs.axi.rresp =
        NPU_MIF_AXI_RESP_OKAY;
    inputs.axi.rlast = 0u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    ...
    inputs.axi.rdata = UINT64_C(0xa1);
    inputs.axi.rlast = 1u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.owner[NPU_MIF_OWNER_DMA].rsp_valid != 0u);
    TEST_CHECK(outputs.owner[NPU_MIF_OWNER_DMA].rsp_last == 0u);
    ...
    inputs.axi.rdata = UINT64_C(0xb0);
    inputs.axi.rlast = 0u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.axi.rready != 0u);
    inputs.axi.rdata = UINT64_C(0xb1);
    inputs.axi.rlast = 1u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.owner[NPU_MIF_OWNER_DMA].rsp_valid != 0u);
    TEST_CHECK(outputs.owner[NPU_MIF_OWNER_DMA].rsp_last == 0u);
...
static int mif_test_burst_limit(void)
{
    ...
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.axi.arvalid != 0u);
    TEST_CHECK(outputs.axi.arlen == 15u);
    first_id = outputs.axi.arid;
    inputs.axi.arready = 0u;
    ...
    npu_mif_cycle_inputs_t *inputs)
{
    ...
    inputs->system_addr_enable = 1u;
    inputs->system_addr_base = UINT64_C(0x020000);
    inputs->system_addr_limit = UINT64_C(0x02fff8);
...
static int mif_test_config_validation(void)
{
    ...
    TEST_CHECK(
        (config.system_addr_limit & UINT64_C(7)) == 0u);
    ...
    invalid = config;
    invalid.system_addr_base =
        NPU_MIF_PA_MASK + UINT64_C(1);
    TEST_CHECK(npu_mif_cycle_config_valid(&invalid) == 0u);
    invalid = config;
    invalid.system_addr_base = UINT64_C(1);
    TEST_CHECK(npu_mif_cycle_config_valid(&invalid) == 0u);
    invalid = config;
    invalid.system_addr_base = UINT64_C(0x1000);
    invalid.system_addr_limit = UINT64_C(0x0ff8);
    TEST_CHECK(npu_mif_cycle_config_valid(&invalid) == 0u);
...
static int mif_test_dynamic_config_snapshot_and_reset(void)
{
    ...
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.owner[NPU_MIF_OWNER_DFU].req_ready != 0u);
    TEST_CHECK(model.config.system_addr_enable != 0u);
    TEST_CHECK(
        model.config.system_addr_base == UINT64_C(0x020000));
    TEST_CHECK(
        model.config.system_addr_limit == UINT64_C(0x02fff8));
    ...
    /* A completes under its original system address snapshot. */
    inputs.tbu.rsp_valid = 1u;
    ...
    TEST_CHECK(outputs.axi.araddr ==
               UINT64_C(0x3000));
    old_axi_id = outputs.axi.arid;
    inputs.axi.arready = 1u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    inputs.axi.arready = 0u;
    inputs.axi.rvalid = 1u;
    inputs.axi.rid = old_axi_id;
    ...
    /* B's TBU request and physical range both use the new snapshot. */
    inputs.tbu.req_ready = 1u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.tbu.req_valid != 0u);
    ...
    /*
     * Accept B's address, then accept bypass request C with config_valid
     * low. C uses the retained system and bypass ranges.
     */
    inputs.axi.arready = 1u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    inputs.axi.arready = 0u;
    ...
    mif_test_drive_dynamic_config(&inputs);
    inputs.system_addr_enable = 1u;
    inputs.system_addr_base = 0u;
    inputs.system_addr_limit = UINT64_MAX;
    inputs.reset_n = 0u;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.mif_idle != 0u);
    TEST_CHECK(outputs.owner[NPU_MIF_OWNER_DFU].req_ready == 0u);
...
static int mif_test_limit_names_final_beat(void)
{
    ...
    npu_mif_cycle_config_default(&config);
    config.system_addr_base = UINT64_C(0x1000);
    config.system_addr_limit = UINT64_C(0x1000);
    config.bypass_enable = 1u;
...
static int mif_test_axi_ingress_illegal_then_recovery(void)
{
     ...
     */
    inputs.axi.bid = 0xfeu;
    inputs.axi.rid = 0xfdu;
    npu_mif_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.axi.bready != 0u);
    TEST_CHECK(outputs.axi.rready != 0u);
...
static int mif_test_write_ready_and_input_buffer(void)
{
     ...
     */
    valid_dma->wvalid = 0u;
    valid_inputs.axi.wready = 1u;
    npu_mif_cycle_step(
        &valid_model, &valid_inputs, &valid_outputs);
    TEST_CHECK(valid_outputs.owner[NPU_MIF_OWNER_DMA].wready ==
               0u);
    TEST_CHECK(valid_outputs.axi.wvalid == 0u);
    TEST_CHECK(valid_model.w_input.valid != 0u);
...
int test_mif_cycle(void)
{
    ...
    line = mif_test_dfu_high_system_address_read();
    if (line != 0) {
        return line;
    }
