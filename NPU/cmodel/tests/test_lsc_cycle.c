...
#if 0
static int lsc_test_submit_cfg(void)
{
    npu_lsc_cycle_config_t config;
    npu_lsc_cycle_t model;
    npu_lsc_cycle_inputs_t inputs;
    npu_lsc_cycle_outputs_t outputs;
    static const uint16_t address[NPU_LSC_SUBMIT_CFG_COUNT] = {
        NPU_LSC_REG_SUBMIT_CFG0,
        NPU_LSC_REG_SUBMIT_CFG1,
        NPU_LSC_REG_SUBMIT_CFG2,
        NPU_LSC_REG_SUBMIT_CFG3,
        NPU_LSC_REG_SUBMIT_CFG4,
        NPU_LSC_REG_SUBMIT_CFG5,
        NPU_LSC_REG_SUBMIT_CFG6,
        NPU_LSC_REG_SUBMIT_CFG7
    };
    uint64_t submit_cfg_value;
    uint64_t read_value;
    uint32_t index;
    ...
    for (index = 0u; index < NPU_LSC_SUBMIT_CFG_COUNT; index++) {
        TEST_CHECK(address[index] ==
                   NPU_LSC_REG_SUBMIT_CFG_BASE + index * 8u);
        TEST_CHECK(
            model.submit_cfg[index] == NPU_LSC_SUBMIT_CFG_RESET);
        TEST_CHECK(
            outputs.submit_cfg[index] ==
            NPU_LSC_SUBMIT_CFG_RESET);
        TEST_CHECK(lsc_read(
                       &model, &inputs, &outputs, 0u,
                       address[index],
                       &read_value) == NPU_LSC_REG_OKAY);
        TEST_CHECK(read_value == NPU_LSC_SUBMIT_CFG_RESET);
    }

    submit_cfg_value =
        (UINT64_C(0x123) <<
         NPU_LSC_SUBMIT_CFG_WAIT0_SHIFT) |
        (UINT64_C(0x456) <<
         NPU_LSC_SUBMIT_CFG_WAIT1_SHIFT) |
        (UINT64_C(0x789) <<
         NPU_LSC_SUBMIT_CFG_SIGNAL_SHIFT) |
        (UINT64_C(0x1d) <<
         NPU_LSC_SUBMIT_CFG_FLAGS_SHIFT) |
        (UINT64_C(0x0a) <<
         NPU_LSC_SUBMIT_CFG_TIMEOUT_SHIFT);

    TEST_CHECK(
        ((submit_cfg_value >> NPU_LSC_SUBMIT_CFG_WAIT0_SHIFT) &
         NPU_LSC_SUBMIT_CFG_EVENT_MASK) == UINT64_C(0x123));
    TEST_CHECK(
        ((submit_cfg_value >> NPU_LSC_SUBMIT_CFG_WAIT1_SHIFT) &
         NPU_LSC_SUBMIT_CFG_EVENT_MASK) == UINT64_C(0x456));
    TEST_CHECK(
        ((submit_cfg_value >> NPU_LSC_SUBMIT_CFG_SIGNAL_SHIFT) &
         NPU_LSC_SUBMIT_CFG_EVENT_MASK) == UINT64_C(0x789));
    TEST_CHECK(
        ((submit_cfg_value >> NPU_LSC_SUBMIT_CFG_FLAGS_SHIFT) &
         NPU_LSC_SUBMIT_CFG_FLAGS_MASK) == UINT64_C(0x1d));
    TEST_CHECK(
        ((submit_cfg_value >> NPU_LSC_SUBMIT_CFG_TIMEOUT_SHIFT) &
         NPU_LSC_SUBMIT_CFG_TIMEOUT_MASK) == UINT64_C(0x0a));

    inputs.reg_req_valid = 1u;
    inputs.reg_req_write = 1u;
    inputs.reg_req_space = 0u;
    inputs.reg_req_addr = NPU_LSC_REG_SUBMIT_CFG0;
    inputs.reg_req_wdata = submit_cfg_value;
    inputs.reg_req_wstrb = 0xffu;
    npu_lsc_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(
        outputs.submit_cfg[0] == NPU_LSC_SUBMIT_CFG_RESET);
    TEST_CHECK(model.submit_cfg[0] == submit_cfg_value);
    inputs.reg_req_valid = 0u;
    npu_lsc_cycle_step(&model, &inputs, &outputs);
    TEST_CHECK(outputs.reg_rsp_valid == 1u);
    TEST_CHECK(outputs.reg_rsp_status == NPU_LSC_REG_OKAY);
    TEST_CHECK(outputs.submit_cfg[0] == submit_cfg_value);
    lsc_clear_response(&model, &inputs, &outputs);
    ...
    TEST_CHECK(lsc_read(
                   ...
                   NPU_LSC_REG_SUBMIT_CFG0,
                   &read_value) == NPU_LSC_REG_OKAY);
    TEST_CHECK(read_value == submit_cfg_value);

    TEST_CHECK(lsc_write(
                   &model, &inputs, &outputs, 0u,
                   NPU_LSC_REG_SUBMIT_CFG1,
                   NPU_LSC_SUBMIT_CFG_RESET,
                   0x03u) == NPU_LSC_REG_SLVERR);
    TEST_CHECK(
        model.submit_cfg[1] == NPU_LSC_SUBMIT_CFG_RESET);
    TEST_CHECK(lsc_write(
                   &model, &inputs, &outputs, 0u,
                   NPU_LSC_REG_SUBMIT_CFG1,
                   NPU_LSC_SUBMIT_CFG_RESET,
                   0x00u) == NPU_LSC_REG_SLVERR);
    TEST_CHECK(
        model.submit_cfg[1] == NPU_LSC_SUBMIT_CFG_RESET);
    ...
    TEST_CHECK(lsc_write(
                   &model, &inputs, &outputs, 0u,
                   NPU_LSC_REG_SUBMIT_CFG0,
                   submit_cfg_value | (UINT64_C(1) << 46u),
                   0xffu) == NPU_LSC_REG_SLVERR);
    TEST_CHECK(model.submit_cfg[0] == submit_cfg_value);
    TEST_CHECK(lsc_write(
                   &model, &inputs, &outputs, 0u,
                   NPU_LSC_REG_SUBMIT_CFG0,
                   submit_cfg_value | NPU_LSC_SUBMIT_CFG_DESC_CRC,
                   0xffu) == NPU_LSC_REG_SLVERR);
    TEST_CHECK(model.submit_cfg[0] == submit_cfg_value);
    ...
    TEST_CHECK(model.submit_cfg[7] == submit_cfg_value);
