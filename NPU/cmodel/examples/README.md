...
第一组编码器的 $X_{\mathrm{next}}$ 送入第二组编码器。两个 `MultiHeadAttention` 都关闭 bias。第一层 Q/K 固定为 0，使这一层对 8 个位置等权重汇集；第二层 Q/K 固定为 0.5 倍的分 head 单位矩阵，使注意力分数随输入内容变化。V、输出投影、前馈网络、LayerNorm 和分类层仍由训练确定。模型共有 14884 个参数，其中 4096 个固定 Q/K 参数不参加训练，另外 10788 个参数参加训练。
...
$c^\ast$ 是真实类别，$c$ 是其他类别编号，$p$ 是 token 位置。模型先使用 Adam、MSE 训练 100 个 epoch，再进行 40 个带 Q5 激活模拟的训练 epoch 和 8 个逐轮权重吸附周期；每组含 16 条语句。训练图会在两个编码器输出处加入小幅噪声，最终导出的标准 Keras 图不含噪声和训练专用的 Q5 模拟节点。导出权重全部落在 $1/32$ 的 Q5 网格点上。
...
- 所有导出权重都能用 INT8 Q5 表示，并且落在 $1/32$ 的网格点上。

固定种子生成的当前 Keras 报告中，训练集和 24 条留出语句的分类正确率均为 1.0。最终分数最小值为 -0.4894032，最大值为 1.0098633，均值为 0.0299303，标准差为 0.4247226；绝对值不小于 0.95 的值有 21/768，INT8 两端值为 0/768，不同 INT8 数值有 32 种，最小第一名分差为 0.6134157。第一层的 768 个注意力行均为等权重，第二层的 768 个注意力行全部为非等权重。
...
| Transformer | loss 1.51194382 | Q5 调整后 loss 0.00684047，训练语句 accuracy 1.0 | 24 条留出语句 loss 0.00546169，accuracy 1.0 | 整句 accuracy 24/24，最小第一名分差 0.6134157 |
...
| Transformer | 24/24 分类正确，最小第一名分差 0.3750000 | 第一探针 MAE 3.4039714 LSB，第二探针 MAE 2.5737305 LSB，最终分数 MAE 2.3307292 LSB | 24/24 与 Keras 类别一致；三个输出的全部元素均通过各自误差限制 |
