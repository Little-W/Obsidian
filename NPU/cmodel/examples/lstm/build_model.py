#!/usr/bin/env python3
"""Train and export the LSTM telemetry regression model."""
...
def build_model() -> keras.Model:
    ...
    return keras.Model(source, output, name="lstm_telemetry_forecast")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    support.atomic_write(
        ...
        (
            json.dumps(
                support.recurrent_graph("lstm", model),
                indent=2,
                sort_keys=True,
                allow_nan=False,
            )
    ...
    support.atomic_write(
        ...
        support.fixture_header(
            "lstm", inputs, targets, reference, metrics
        ),
    )
    report = {
        "format_version": 1,
    ...
    print(
        f"kind=lstm tensorflow={tf.__version__} "
