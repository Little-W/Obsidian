#!/usr/bin/env python3
"""Train and export the SimpleRNN telemetry regression model."""
...
def build_model() -> keras.Model:
    ...
    return keras.Model(source, output, name="rnn_telemetry_forecast")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    support.atomic_write(
        ...
        (
            json.dumps(
                support.recurrent_graph("rnn", model),
                indent=2,
                sort_keys=True,
                allow_nan=False,
            )
    ...
    support.atomic_write(
        ...
        support.fixture_header(
            "rnn", inputs, targets, reference, metrics
        ),
    )
    report = {
        "format_version": 1,
    ...
    print(
        f"kind=rnn tensorflow={tf.__version__} keras={keras.__version__} "
