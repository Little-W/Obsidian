...
达到目标需要以更大的输出区域组织数据。例如，对 64×64 输出区域和 4-K 小段，先取入 64×4 的 A 面板与 4×64 的 B 面板，共 512 B；随后连续完成其中 16 个 16×16 输出块的计算。A 面板被 4 组列块共用，B 面板被 4 组行块共用。4 个 64-bit 读取端口可在 16 个计算拍内提供这 512 B，因而能够与这 16 个阵列计算拍重叠。为保存 16 个输出块跨 K 小段的部分和，还需要可同时保留 64×64 个 INT32 值的部分和存储区，即至少 16 KiB；当前 Outer PE 只保留一个 16×16 输出块的累加器，尚不具备这种能力。
