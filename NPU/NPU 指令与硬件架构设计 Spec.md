...
| 模型 | RTL 仿真时间 | NPU 指令数 | 批次 | NPU 写回字节数 | 最终输出个数 | 结果 |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| Transformer | 23,695,000 ns | 8 | 1 | 144 | 12 | 通过 |
| RNN | 21,595,000 ns | 10 | 2 | 64 | 16 | 通过 |
| GRU | 28,235,000 ns | 10 | 2 | 192 | 16 | 通过 |
| LSTM | 31,365,000 ns | 10 | 2 | 256 | 32 | 通过 |
| CNN | 20,815,000 ns | 10 | 2 | 32 | 8 | 通过 |
...
`tb_npu_matrix_multi_dtype_outer_engine` 已增加完整 16×16、INT8、`K=511` 的直接阵列测试。该测试直接向 Matrix 子模块提交内部 Task Context，不经过 TaskScheduler，因此其 `task_cycles` 尚未包含指令接收、任务表选择和 Event 操作；它已经包含 Outer 控制器、单端口 L1 请求/响应、直接阵列发射和 C 写回的全部周期。直接阵列的 INT8 累加使用逻辑 INT24，完整带符号 INT8 范围内 `511×(-128)×(-128)=8,372,224`，仍可由 signed INT24 表示；`K=512` 的结果为 8,388,608，比 signed INT24 的最大值 8,388,607 大 1，因此必须走精确的 Scalar 路径。公开矩阵指令的 `K-1` 字段为 6 bit，单条指令最多表示 `K=64`；该内部 Task Context 测试不能当作一条公开指令的端到端结果。

| 用例 | `task_cycles` | `array_compute_window_cycles` | 阵列时间利用率 | 阵列空间利用率 |
| --- | ---: | ---: | ---: | ---: |
| INT8，16×16×64 | 3,497 | 16 | 0.458% | 100% |
| INT8，16×16×511 | 21,505 | 128 | 0.595% | 100% |
...
每个直接阵列计算拍中，16×16 个 PE 都被使能，所以空间利用率为 100%。但控制器仍按 A 读、A 返回、B 读、B 返回的次序装载每一个元素；在此期间阵列没有计算。因此，当前结构远未达到 85% 的任务级目标。即使不计 TaskScheduler，`K=511` 用例已有 99.405% 的任务周期不在阵列计算窗口内；加入完整指令调度后只会进一步降低该比例。

要使该指标达到目标，不能只把 16×16 输出块的按元素读取改成一次面板读取。一个 INT8 的 4-K 计算拍需要 64 B 的 A 面板和 64 B 的 B 面板，共 128 B；即使有 4 个独立 64-bit Matrix 读端口，也至少需要 4 个周期装入这些数据。因此，如果每次只计算一个 16×16 输出块，阵列时间利用率最多约为 25%，仍不足 85%。

达到目标需要以更大的输出区域组织数据。例如，对 64×64 输出区域和 4-K 小段，先取入 64×4 的 A 面板与 4×64 的 B 面板，共 512 B；随后连续完成其中 16 个 16×16 输出块的计算。A 面板被 4 组列块共用，B 面板被 4 组行块共用。4 个 64-bit 读取端口可在 16 个计算拍内提供这 512 B，因而能够与这 16 个阵列计算拍重叠。为保存 16 个输出块跨 K 小段的部分和，还需要可同时保留 64×64 个 INT32 值的部分和存储区，即至少 16 KiB；当前 Outer PE 只保留一个 16×16 输出块的累加器，尚不具备这种能力。对于 `K>64`，编译器还必须把多个公开 GEMM 任务组成一个宏块，并让直接阵列路径正确支持本地部分和的建立、保留和最终写回；当前 TVM 生成的这些 K 分块任务仍走 Scalar 路径。

硬件应提供 4 个可独立访问的 64-bit Matrix bank 读取端口、独立结果写端口、A/B 双缓冲和多输出块部分和存储区。编译器应为 A、B、C 安排不冲突的 bank 位置，并把当前 K 小段的计算与下一小段的面板读取交叠。L1 仲裁器还应允许 Matrix 面板读、部分和访问和结果写回在同一时段推进。该表记录的是当前测量结果，不表示这些硬件已经完成。
