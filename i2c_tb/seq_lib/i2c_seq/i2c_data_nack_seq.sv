class i2c_data_nack_seq extends i2c_basic_sequence;
  `uvm_object_utils(i2c_data_nack_seq)

  function new(string name = "i2c_data_nack_seq");
    super.new(name);
    write_data = '{8'h55, 8'hbe, 8'hef};
...
`else
    `uvm_fatal("I2C_SVT_REQUIRED", "Data NACK test requires BMS_USE_SVT_VIP")
`endif
  endtask
endclass
