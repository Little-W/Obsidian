...
class apb_shared_cfg extends uvm_object ;
  ...
  function new(string name = "apb_shared_cfg_inst");
    ...
    master_cfg.slave_cfg[0].is_active = 1;

    /** Directed APB sequences are used in this TB; no RAL model is configured. */
    master_cfg.uvm_reg_enable = 0;
    ...
    slave_cfg.enable_complex_memory_map =0;
    slave_cfg.slave_addr_allocation_enable = 0;
    slave_cfg.initialize_output_signals_at_start=1;

    /** Slave setup */
    slave_cfg.create_sub_cfgs(`APB_SLAVES_NUM);

    /** Enable protocol file generation for Protocol Analyzer */

    //slave_cfg.trace_enable = 1;
    for (int i=0;i<`APB_SLAVES_NUM;i++)begin
    slave_cfg.slave_cfg[i].enable_xml_gen = 1;
    slave_cfg.slave_cfg[i].is_active = 1;
    slave_cfg.slave_cfg[i].default_pready = 1;
    slave_cfg.slave_cfg[i].transaction_coverage_enable = 1;
    slave_cfg.slave_cfg[i].pauser_width = 0;
    //slave_cfg.slave_cfg[i].trace_enable = 1;
    slave_cfg.slave_cfg[i].protocol_checks_coverage_enable = 1;
//    this.apb_sys_cfg[114].slave_addr_ranges = new[1];

    //this.apb_sys_cfg[114].slave_addr_range[0] = new();
    //this.apb_sys_cfg[114].slave_addr_range[0].start_addr = 32'h00_0001_0000 ;
    //this.apb_sys_cfg[114].slave_addr_range[0].end_addr = 32'h 00_0001_1FFF;
    //this.apb_sys_cfg[114].slave_addr_range[0].slave_id = 0;
    end
   slave_cfg.set_addr_range(0,40'h00_0000,40'hffff_ffff);
  // slave_cfg.set_addr_range(1,40'h20_1000,40'h20_1FFF);
  // slave_cfg.set_addr_range(2,40'h20_2000,40'h20_2FFF);
  // slave_cfg.set_addr_range(3,40'h20_3000,40'h20_3FFF);
  // slave_cfg.set_addr_range(4,40'h20_4000,40'h20_4FFF);
  // slave_cfg.set_addr_range(5,40'h20_5000,40'h20_5FFF);
  // slave_cfg.set_addr_range(6,40'h20_6000,40'h20_6FFF);
  // slave_cfg.set_addr_range(7,40'h20_7000,40'h20_7FFF);
  // slave_cfg.set_addr_range(8,40'h20_8000,40'h20_8FFF);
  // slave_cfg.set_addr_range(9,40'h20_9000,40'h20_9FFF);
  endfunction
  /**
