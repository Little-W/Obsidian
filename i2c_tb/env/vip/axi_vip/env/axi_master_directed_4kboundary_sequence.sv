...
/**
 * Abstract:
 * axi_master_directed_4kboundary_sequence is used by test to provide initiator
...
`ifndef GUARD_AXI_MASTER_DIRECTED_S4KBOUNDARY_EQUENCE_SV
`define GUARD_AXI_MASTER_DIRECTED_S4KBOUNDARY_EQUENCE_SV

class axi_master_directed_4kboundary_sequence extends svt_axi_master_base_sequence;

  /** Parameter that controls the number of transactions that will be generated */
   int unsigned sequence_length = 5;
  ...
  `uvm_object_utils(axi_master_directed_4kboundary_sequence)

  /** Class Constructor */
  function new(string name="axi_master_directed_4kboundary_sequence");
    super.new(name);
  endfunction
  ...
  virtual task body();
    ...
    for(int i = 0; i < sequence_length; i++) begin
      ...
      write_tran.addr         = 32'h0000_fefe | ('h100 << i);
...
`endif
      ...
      read_tran.addr         = 32'h0000_fefe | ('h100 << i);
...
virtual task post_body();
 super.post_body();
  if (`SVT_AXI_TRANSACTION_ADDR_RANGE_NUM_LSB_BITS >12 && `SVT_AXI_TRANSACTION_4K_ADDR_RANGE > 4096) begin
     `uvm_info("post_body", $sformatf("SVT_AXI_TRANSACTION_ADDR_RANGE_NUM_LSB_BITS is %0d and SVT_AXI_TRANSACTION_4K_ADDR_RANGE is %d  ", `SVT_AXI_TRANSACTION_ADDR_RANGE_NUM_LSB_BITS, `SVT_AXI_TRANSACTION_4K_ADDR_RANGE ), UVM_LOW);

end
else if (`SVT_AXI_TRANSACTION_ADDR_RANGE_NUM_LSB_BITS  <= 12 && `SVT_AXI_TRANSACTION_4K_ADDR_RANGE <= 4096) begin
     `uvm_info("post_body", $sformatf("SVT_AXI_TRANSACTION_ADDR_RANGE_NUM_LSB_BITS is %d and SVT_AXI_TRANSACTION_4K_ADDR_RANGE is %d ", `SVT_AXI_TRANSACTION_ADDR_RANGE_NUM_LSB_BITS, `SVT_AXI_TRANSACTION_4K_ADDR_RANGE), UVM_LOW);

end
else begin
     `uvm_error ("post_body"," The 4K boundary crossing values are inconsistent");
  end
endtask

endclass: axi_master_directed_4kboundary_sequence

`endif // GUARD_AXI_MASTER_DIRECTED_S4KBOUNDARY_EQUENCE_SV
