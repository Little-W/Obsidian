...
3. Organize the report around the technical question. Keep sourced facts, calculations, measured results, engineering judgments, and recommendations distinguishable.
4. Prefer primary and authoritative sources. State conditions, scope, uncertainty, and limitations when they affect a claim.
5. Implement Word formatting with named styles and semantic document structure rather than manual spaces, repeated blank paragraphs, or appearance-only formatting.
6. Verify the actual DOCX, update fields when possible, render it to PDF, and inspect the rendered pages. A successful converter exit is not proof of a correct deliverable.
